...
def fourier_featurize(x,numSinusoidals=20):
...
def combination_featurize(x,numSinusoidals=20,polynomialDegree=5):
    # if x is a vector of 5 input values and polynomialDegree=20
    # x_featurized will be 5x21, the first one is the intercept
    ...
    for i in range(polynomialDegree+1):
        x_featurized[:,i]=x**i
    for i in range(numSinusoidals):
        x_featurized[:,2*i+polynomialDegree+1]=np.cos((i+1)*x)
        x_featurized[:,2*i+polynomialDegree+2]=np.sin((i+1)*x)
    return x_featurized
