...
def fourier_featurize(x,numSinusoidals=20,period=2*np.pi,phaseShift=0):
    ...
    for i in range(numSinusoidals):
        x_featurized[:,2*i+1]=np.cos((i+1)*coefficient*x-phaseShift)
        x_featurized[:,2*i+2]=np.sin((i+1)*coefficient*x-phaseShift)
    return x_featurized
...
def combination_featurize(x,numSinusoidals=20,period=2*np.pi,polynomialDegree=5,phaseShift=0):
    # if x is a vector of 5 input values and polynomialDegree=20
    # x_featurized will be 5x21, the first one is the intercept
    # period is the orbit period in seconds
    coefficient=2*np.pi/period
    ...
    for i in range(polynomialDegree+1):
        x_featurized[:,i]=x**i
    for i in range(numSinusoidals):
        x_featurized[:,2*i+polynomialDegree+1]=np.cos((i+1)*coefficient*x-phaseShift)
        x_featurized[:,2*i+polynomialDegree+2]=np.sin((i+1)*coefficient*x-phaseShift)
    return x_featurized
